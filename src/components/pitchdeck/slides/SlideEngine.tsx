    ...
    <DeckSlide id="engine" number={4}>
      <div className="flex flex-col items-center gap-6">
        <h2 className="font-display text-3xl md:text-5xl font-bold text-white text-center">
          What powers the <span className="text-[#047857]">answers</span>
        </h2>
            ...
            {dataSources.map((src) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[#047857] text-xs font-bold">{src.label}</span>
                  <span className="font-mono text-[#047857] text-[9px] opacity-70">{src.stat}</span>
                </div>
        ...
        {/* Arrow down */}
        <div className="flex flex-col items-center gap-0 text-[#047857]">
          <div className="w-px h-4 bg-[#047857] opacity-40" />
            ...
            {processingLayers.map((layer, i) => (
              <div key={layer.label} className="flex items-center gap-1 flex-1">
                <div className="glass-card terminal-green rounded-lg p-3 flex flex-col items-center text-center gap-1 w-full">
                  <span className="font-mono text-[#047857] font-bold text-xs tracking-wider">
                    {layer.label}
                  </span>
                  <span className="text-gray-500 text-[10px]">{layer.description}</span>
                </div>
                {i < processingLayers.length - 1 && (
                  <span className="text-[#047857] text-sm font-bold shrink-0 hidden md:block">&rarr;</span>
                )}
              </div>
            ))}
          </div>
          ...
          {[
            ...
            <div key={s.label} className="flex items-center gap-6">
              ...
              <div className="text-center">
                <span className="font-mono text-[#047857] text-sm font-bold">{s.value}</span>
