  ...
  return (
    <div style={{ background: '#FAFAF8', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'Inter, system-ui, sans-serif', color: '#0B1220' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(250,250,248,0.85)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)', borderBottom: '1px solid #E7E9EC' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: isMobile ? '0 20px' : '0 32px', height: 62, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxSizing: 'border-box' }}>
...
export function PageHero({ pill, titleA, titleB, sub }: { pill: string; titleA: string; titleB?: string; sub: string }) {
  const isMobile = useIsMobile(760);
  return (
    <section style={{ textAlign: 'center', padding: isMobile ? '52px 22px 30px' : '80px 32px 40px', maxWidth: 820, margin: '0 auto' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-heading)', margin: isMobile ? '18px 0 0' : '22px 0 0', fontSize: isMobile ? 33 : 52, fontWeight: 800, lineHeight: 1.06, letterSpacing: '-0.02em', color: '#0B1220' }}>
      ...
      <p style={{ margin: '18px auto 0', maxWidth: 620, fontSize: isMobile ? 15.5 : 17, lineHeight: 1.55, color: '#475467' }}>{sub}</p>
    </section>
  );
}
